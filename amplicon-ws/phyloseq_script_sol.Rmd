...
```{r}
tree <- read_tree_greengenes(sprintf("%s/tree.nwk", INPUT_DIR))
```
...
```{r}
at_least_4k <- prune_samples(sample_sums(physeq) >= 4000, physeq)
sample_sums(at_least_4k)
```
...
**No, the gut is clearly distinct.**
...
```{r}
no_gut <- subset_samples(physeq, body.site != "gut")
no_gut
```
...
**The relative abundance of each taxon is normalized within each sample and doesn't take into account other samples. We can thus use all available data.**
...
# relative abundance
gut_counts <- taxa_sums(gut)
relative_abundance_gut <- calculate_relative_abundance(gut_counts)

# prune suspected taxonomic outliers
abundant_gut <- relative_abundance_gut > 0.0005
abundant_gut_taxa <- prune_taxa(abundant_gut, gut)

# aggregate by Genus
abundant_gut_genera <- tax_glom(abundant_gut_taxa, taxrank = "Genus")
abundant_gut_genera
```
...
# filter to tongue
tongue <- subset_samples(physeq, body.site == "tongue")

# remove low abundance taxa with RA <= 0.0005
tongue_counts <- taxa_sums(tongue)
relative_abundance_tongue <- calculate_relative_abundance(tongue_counts)
abundant_tongue <- relative_abundance_tongue > 0.0005
abundant_tongue_taxa <- prune_taxa(abundant_tongue, tongue)

# aggregate
family <- tax_glom(abundant_tongue_taxa, taxrank = "Family")
...
```{r}
# to deseq
deseq_family <- phyloseq_to_deseq2(family, ~ reported.antibiotic.usage)
family_geo_means <- apply(counts(deseq_family), 1, calculate_gm_mean)
deseq_family <- estimateSizeFactors(deseq_family, geoMeans = family_geo_means)
deseq_family <- DESeq(deseq_family, fitType = "local")

# to dataframe
familiy_diff_abund <- results(deseq_family)
family_df <- as.data.frame(tax_table(family))
family_df <- filter(family_df, Family != "f__Mitochondria")
family_df <- filter(family_df, Family != "f__Chloroplast")

# filter by "significance" (alpha = 0.5)
significant_family <- as.data.frame(familiy_diff_abund)
significant_family <- filter(significant_family, padj < 0.5)
...
```{r}
# plot
significant_family_p <- mutate(significant_family,
                          Family = factor(Family, levels = Family))
ggplot(significant_family_p, aes(x = log2FoldChange, y = Family)) +
  geom_bar(stat = "identity") +
  labs(title = "Differential abundant genera",
       x = expression(log[2]~fold~change),
       y = "Family") +
...
relative_abundance_gut <- calculate_relative_abundance(taxa_sums(gut))
abundant_gut <- relative_abundance_gut > 0.0005
abundant_gut_taxa <- prune_taxa(abundant_gut, gut)

# get relative abundance
gut_RA <- transform_sample_counts(abundant_gut_taxa, calculate_relative_abundance)

# aggregate by Genus and subset to only "g__Alistipes"
abundant_gut_RA_genera <- tax_glom(gut_RA, taxrank = "Genus")
alistipes <- subset_taxa(abundant_gut_RA_genera, Genus == "g__Alistipes")
```
...
relative_abundance <- calculate_relative_abundance(counts)
abundant <- relative_abundance > 0.0005
abundant_taxa <- prune_taxa(abundant, physeq)

# get relative abundance
taxa_RA <- transform_sample_counts(abundant_taxa, calculate_relative_abundance)

# aggregate by Genus and subset to only "g__Alistipes"
genera <- tax_glom(taxa_RA, taxrank = "Genus")
strep <- subset_taxa(genera, Genus == "g__Streptococcus")

strep_l <- psmelt(strep)
strep_l

ggplot(strep_l, aes(x = body.site, y = Abundance, fill = body.site)) +
  geom_boxplot() +
  labs(title = "Relative abundance of Streptococcus",
       x     = "Body Site",
       y     = "Relative abundance")
```
...
```{r}
ordj <- ordinate(physeq_rar, method = "PCoA", distance = "jaccard")
plot_ordination(physeq_rar,
                ordj,
                color = "subject") +
  # Define title of plot
